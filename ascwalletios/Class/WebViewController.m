...
//    [self deleteWebCache];
    ...
    NSString * string =  @"https://s2.j10kb.cn/index.html?t=";
//     NSString * string =  @" http://192.168.1.24:8080/index.html?t=";
//      NSString * string =  @" http://www.baidu.com/?t=";
    ...
    self.progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, 50, self.view.bounds.size.width, 2)];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    if ([message.name isEqualToString:@"getregisterid"]) {
        
        NSLog(@"getregisterid");
        extern NSString *registerID;
        NSLog(@"getregisterid %@",registerID);
        NSString *jsString = [NSString stringWithFormat:@"localStorage.setItem('registerid', '%@')", registerID];
        [self runJs:jsString];
        NSString *jsString2 = [NSString stringWithFormat:@"getRegisterId('%@')", registerID];
        [self runJs:jsString2];
        
        };
    //  [self deleteWebCache];
    if ([message.name isEqualToString:@"clearCache"]) {
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
      self.network=YES;
    NSLog(@"5---didFinishNavigation----页面加载完成之后调用");
    [self.progressView removeFromSuperview];
...
//    }
    ...
    if ([urlString hasPrefix:@"ascaction:"]) {
...
-(void)saveImageToAlbum{
    NSString *shareImage = @"htter1/bg@2x.png";
...
{
    NSLog(@"checkVersion");
    NSString *localVersion = [[[NSBundle mainBundle]infoDictionary] objectForKey:@"CFBundleShortVersionString"]; NSLog(@"当前版本为：%@",localVersion);

    
//    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
//    [manager POST:@"http://wallet.ascchain.com/download/version.json" parameters:nil success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
...
//}
    ...
    [[AFHTTPSessionManager manager] GET:@"http://wallet.ascchain.com/download/version.json" parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
//        NSLog(@"responseObject======%@", responseObject);
        if([responseObject isKindOfClass:[NSDictionary class]]){
            NSDictionary *json = responseObject;
            NSArray *array = responseObject[@"results"];
            NSDictionary *dict = [array lastObject];
            NSLog(@"新版本为：%@", dict[@"version"]);
             int newVersion = [ dict[@"version"] intValue];
//            if(json != nil){
//                int newVersion = [json[@"version"] intValue];
//                NSLog(@"get version: %d",newVersion);
//                NSString *localVersion = [[[NSBundle mainBundle]infoDictionary] objectForKey:@"CFBundleVersion"];
                NSString *msg =[NSString stringWithFormat:@"您的版本是V%@，发现新版本V%@，是否下载新版本？\n 更新内容：%@",localVersion,  dict[@"version"], dict[@"releaseNotes"]];
                if (newVersion > [localVersion intValue])
                {
